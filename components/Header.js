...
import {noHiddenArticles} from '../reducers/hiddenArticles';
...
function Header() {
	...
	const handleRegister = () => {
		fetch('https://morning-news-backend-gilt-rho.vercel.app/users/signup', {
			method: 'POST',
	...
	const handleConnection = () => {
		fetch('https://morning-news-backend-gilt-rho.vercel.app/users/signin', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
	...
	if (user.token) {
		userSection = (
			<div className={styles.logoutSection}>
				...
				<FontAwesomeIcon icon={faEye} className={styles.userSection} onClick={() => noHidden()}/>
			</div>
		);
	} else {
		if (isModalVisible) {
